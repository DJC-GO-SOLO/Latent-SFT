...
if __name__ == '__main__':
    ...
    data_hard = read_jsonl('../data/GSM8k-hard-test.jsonl')
    data_svamp = read_jsonl('../data/svamp-test.jsonl')
    dara_multiarith = read_jsonl('../data/multiarith-test.jsonl')

    ## hard
    gen_texts = model.gen(data_hard)
    for i in range(len(data_hard)):
        data_hard[i]['prediction'] = gen_texts[i][0]

    token_nums = [i[1] for i in gen_texts]
    avg_token_num_hard = sum(token_nums) / len(token_nums)

    ## svamp
    gen_texts = model.gen(data_svamp)
    for i in range(len(data_svamp)):
        data_svamp[i]['prediction'] = gen_texts[i][0]

    token_nums = [i[1] for i in gen_texts]
    avg_token_num_svamp = sum(token_nums) / len(token_nums)

    ## multiarith
    gen_texts = model.gen(dara_multiarith)
    for i in range(len(dara_multiarith)):
        dara_multiarith[i]['prediction'] = gen_texts[i][0]

    token_nums = [i[1] for i in gen_texts]
    avg_token_num_multiarith = sum(token_nums) / len(token_nums)
    ...
    ## evaluate
    for i in range(len(data_hard)):
        pred = extract_answer(data_hard[i]['prediction'])
        results_hard.append(check_is_correct(pred, data_hard[i]["answer"]))

    for i in range(len(data_svamp)):
        pred = extract_answer(data_svamp[i]['prediction'])
        results_svamp.append(check_is_correct(pred, data_svamp[i]["answer"]))

    for i in range(len(dara_multiarith)):
        pred = extract_answer(dara_multiarith[i]['prediction'])
        results_multiarith.append(check_is_correct(pred, dara_multiarith[i]["answer"]))
    
    print('=======================================================')
    print(f'GSM8k Hard Scores: {sum(results_hard)/len(results_hard)}  AVG Tokens: {avg_token_num_hard}')
    print(f'Svamp Scores: {sum(results_svamp)/len(results_svamp)}  AVG Tokens: {avg_token_num_svamp}')
    print(f'MultiArith Scores: {sum(results_multiarith)/len(results_multiarith)}  AVG Tokens: {avg_token_num_multiarith}')
    ...
    write_jsonl(data_hard,os.path.join(args.save_path,f'{check_point}_{args.max_new_tokens}_hard_result.jsonl'))
    write_jsonl(data_svamp,os.path.join(args.save_path,f'{check_point}_{args.max_new_tokens}_svamp_result.jsonl'))
    write_jsonl(dara_multiarith,os.path.join(args.save_path,f'{check_point}_{args.max_new_tokens}_multiarith_result.jsonl'))
    with open(os.path.join(args.save_path,'eval_result_OOD.txt'), "a", encoding="utf-8") as f:
        f.write('=======================================================\n')
        f.write(f'GSM8k Hard Scores: {sum(results_hard)/len(results_hard)}  AVG Tokens: {avg_token_num_hard}  Svamp Scores: {sum(results_svamp)/len(results_svamp)}  AVG Tokens: {avg_token_num_svamp} MultiArith Scores: {sum(results_multiarith)/len(results_multiarith)}  AVG Tokens: {avg_token_num_multiarith} check_point: {check_point} mex_new_tokens: {args.max_new_tokens}'+'\n')

...
class Req:
    ...
    def __init__(
        ...
        if self.enable_latent:
            ...
            # 正确初始化方式
            # self.prob = torch.empty(
            #     1024,  # 注意：直接传尺寸数字，不要用元组
            ...
            self.prob = torch.full(
                (vocab_size,),
                float('nan'),
                dtype=torch.bfloat16,         # 和模型 forward 的 dtype 一致（fp16/bf16）
                device=torch.device('cuda:0'),       # 和当前进程的 device 一致，不要写死 'cuda:0'
            )
            # self.prob = None
    ...
    def update_latent_info(self, logits_output, index):
        ...
        self.prob = logits_output.probs[index].clone()
        ...
        if self.sampling_params.latent_mode:
            ...
            if self.sampling_params.latent_end_token_id == self.output_ids[-1]:
                ...
                self.prob.zero_()
                self.prob[self.sampling_params.latent_end_token_id] = 1.0
                # print(self.origin_input_text)
        else:
            if last_token_id is not None and 0 <= last_token_id < self.prob.numel():
                self.prob.zero_()
                self.prob[last_token_id] = 1.0

        # 仅在未完成时记录latent信息 吞吐量太大，memory会爆掉，先不存
        # if not self.finished():
        #     self.output_prob_list_tmp.append(self.prob)

    # def get_output_prob_list(self):
    #     if self.output_prob_list_tmp:
    #         self.output_prob_list.extend(torch.stack(self.output_prob_list_tmp, dim=0).cpu().tolist())
    #         self.output_prob_list_tmp = []
    #     return self.output_prob_list
    ...
    def get_model_worker_batch(self) -> ModelWorkerBatch:
        ...
        if self.model_config.enable_latent:
            if self.enable_overlap or self.forward_mode.is_decode():
                probs = torch.stack([req.prob for req in self.reqs])

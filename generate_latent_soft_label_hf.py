...
class MultiprocessTransformerWrapper:
    ...
    def _gen_per_process(
        ...
        model.latent_token_ids = model.tokenizer(['<think>','</think>'], add_special_tokens=False)['input_ids']
        model.compress_token = '<|compress_token|>' 
        model.compress_token_id = model.tokenizer.convert_tokens_to_ids(model.compress_token)  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default='../output/stage1decoderresults/llama3.2-1b-stage1-encoder/best_hf', type=str)
    parser.add_argument('--decoder_path', default='../sft_llama32_model/', type=str)
    parser.add_argument('--save_path', default='../output/stage1decoderresults/llama3.2-1b-stage1-encoder/best_hf/train_latent_soft_label', type=str)
    ...
    for batch in tqdm(chunked(data, batch_size), total=(len(data) + batch_size - 1) // batch_size):
        ...
        latent_states = [item for sublist in latent_states for item in sublist] 

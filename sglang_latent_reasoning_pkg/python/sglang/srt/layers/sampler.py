...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        if enable_latent:
            ## 获取batch所有的probs
            latent_probs = torch.softmax(logits, dim=-1)
            logits_output.probs = latent_probs
            #获取argmax batch_next_token_ids
            # latent_batch_next_token_ids = torch.argmax(logits, -1)
        # ==========
        ...
        # latent mode取argmax最为下一个token id，此处进行回填
        # mask = sampling_info.latent_modes == True

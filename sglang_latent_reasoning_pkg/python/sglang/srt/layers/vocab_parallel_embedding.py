...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    # ==========
    def weighted_forward(self, topk_probs: torch.Tensor, topk_indices: torch.Tensor = None) -> torch.Tensor:
        """
        单卡：若 topk_indices is None，则认为是稠密概率（通常 K=|V|），
        直接 probs @ W_full；否则走原来的稀疏 Top-K 路径。
        """
        probs = topk_probs.to(self.weight.dtype)

        if topk_indices is None:
            if getattr(self, "num_added_embeddings", None) is not None:
                if self.num_added_embeddings != 0:
                    raise RuntimeError(
                        f"dense weighted_forward 仅支持无 added 词表，"
                        f"当前 num_added_embeddings={self.num_added_embeddings}"
                    )
            if getattr(self, "shard_indices", None) is not None:
                if self.shard_indices.num_added_elements != 0:
                    raise RuntimeError(
                        f"dense weighted_forward 仅支持无 added 词表，"
                        f"当前 shard_added={self.shard_indices.num_added_elements}"
                    )
            # 期望 probs 的最后一维是“全局 token_id 顺序”的长度 V（不含 padding）
            V = probs.size(-1)
            # 将 probs 对齐到真实 vocab 长度（self.num_embeddings），避免与 padding 行相乘
            if V != self.num_embeddings:
                # 若上游传了 padded 长度，裁掉；若更短（不常见），补零
                if V > self.num_embeddings:
                    probs = probs[:, : self.num_embeddings]
                else:
                    import torch.nn.functional as F
                    probs = F.pad(probs, (0, self.num_embeddings - V))

            # （可选但推荐）数值安全归一化
            # probs = probs / probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)

            # W_full: [V, D] —— 去掉 padding 的实际词表权重
            W_full = self.weight[: self.num_embeddings, :]  # [V, D]
            # 直接 matmul：F.linear(x, W^T) = x @ W
            out = torch.nn.functional.linear(probs, W_full.t())  # [B, D]
            
            return out

        # -------- 稀疏 Top-K 路径（保持你现有逻辑，K 小内存可控） --------
        # 归一化（若上游已归一化可省，但为稳妥建议保留）
        probs = probs / probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)
        assert probs.shape == topk_indices.shape, "topk_probs and topk_indices must have same shape."

        # 小 K 情况下用 gather 性能很好，内存也可控
        topk_embeddings = self.quant_method.embedding(self, topk_indices.long())  # [B, K, D]
        hidden_states = torch.sum(probs.unsqueeze(-1) * topk_embeddings, dim=1, dtype=topk_embeddings.dtype)  # [B, D]
        return hidden_states

    def weighted_forward_tp(self, topk_probs: torch.Tensor, topk_indices: torch.Tensor = None) -> torch.Tensor:
        """
        TP：若 topk_indices is None，则认为是稠密概率（通常 K=|V|），
        在本 rank 上切片 (base + added) 后分别做局部 matmul，再 all-reduce 求和。
        否则走原来的稀疏 Top-K 路径（带 shard mask）。
        """
        probs = topk_probs.to(self.weight.dtype)

        if topk_indices is None:
            s = self.shard_indices
            # 期望 probs 的最后一维是“全局 token_id 顺序”的 V（不含 padding）
            V = probs.size(-1)
            if V != self.num_embeddings:
                # 若上游传了 padded 长度，裁掉；若更短（不常见），补零
                if V > self.num_embeddings:
            ...
            # 本 rank 的全局切片（base 段 + added 段），注意这些切片都是 “真实 token”，不含 padding
            probs_base  = probs[:, s.org_vocab_start_index   : s.org_vocab_end_index]    # [B, n_org]
            probs_added = probs[:, s.added_vocab_start_index : s.added_vocab_end_index]  # [B, n_add]

            # 本地权重子矩阵（按类 docstring 的布局：base | base_pad | added | added_pad）
            W = self.weight  # [V_shard_padded, D]
            W_base  = W[: s.num_org_elements, :]  # [n_org, D]
            W_added = W[s.num_org_elements_padded : s.num_org_elements_padded + s.num_added_elements, :]  # [n_add, D]

            # 局部 matmul（不与 padding 相乘）
            out = torch.nn.functional.linear(probs_base,  W_base.t())   # [B, D]
            if s.num_added_elements > 0:
                out = out + torch.nn.functional.linear(probs_added, W_added.t())

            # 汇总各 rank 的期望嵌入
            out = tensor_model_parallel_all_reduce(out)  # [B, D]
            return out
        ...
        topk_embeddings.masked_fill_(input_mask, 0)  # 无效索引置零
        hidden_states_parallel = torch.sum(probs.unsqueeze(-1) * topk_embeddings, dim=1, dtype=topk_embeddings.dtype)  # [B, D]
